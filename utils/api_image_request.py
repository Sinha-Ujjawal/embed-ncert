...
def api_image_request(
    ...
    **params,
    ...
    if not r.ok:
        logger.error(f'Error calling the API. Response was {r.text}')
    r.raise_for_status()
...
def api_image_request_streaming(
    ...
    **params,
    ...
    logger.debug(f'API streaming request payload: {json.dumps(payload, indent=2)}')
    ...
    with requests.post(str(url), headers=hdrs, json=payload, timeout=timeout, stream=True) as r:
        if not r.ok:
            logger.error(f'Error calling the API {url} in streaming mode. Response was {r.text}')
        r.raise_for_status()
        ...
        for raw_line in r.iter_lines(decode_unicode=True):
            ...
            try:
                ...
                logger.debug('Skipping non-JSON SSE chunk: %r', data[:200])
                continue
            ...
            try:
                ...
                logger.debug('Unexpected SSE chunk shape: %s', e)
                piece = ''
